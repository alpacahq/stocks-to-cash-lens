...
// ranging from 3.5 years ago to fifteen minutes ago.
function sliderToDate(sliderVal) {
    const threeAndAHalfYearsAgo = 60*60*24*365*3.5*1000;
    const fifteenMinsAgo = 15*60*1000;
    offset = sliderVal * (threeAndAHalfYearsAgo-fifteenMinsAgo) + fifteenMinsAgo
    return offset
}
...
function updateStockPrice(sliderVal) {
    ...
    getBars(symbol, dateBefore.toISOString(), dateOfStockPrice.toISOString(), limit=50, undefined, "1Hour", "all", function(err, body) {
        if (err) {
            ...
            script.t.text = "$SPOT = $" + String(highPrice);
